...
public class Employee {
	...
	public static void main(String[] args) {
		...
		while (true) {
			System.out.print("input user inf -->>");
			String inputEmp = scanner.nextLine();
			if (inputEmp.isBlank())
				break;
			String[] data = inputEmp.split("\\s+");
			int id = Integer.parseInt(data[0]);
			int salary = Integer.parseInt(data[2]);
			emps[empIdx++] = new Employee(id, data[1], salary);
		}
		...
		for (Employee emp : emps) {
			if (emp == null)
				break;
			System.out.printf("%s의 연봉은 %,d원, 인상분은%,d원입니다.%n", emp, emp.getAnnualSalary(),
