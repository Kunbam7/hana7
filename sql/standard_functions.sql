...
select dname, AES_ENCRYPT(dname, '암호키') from Dept; -- AES_DECRYPT, 암호화
select dname, AES_DECRYPT(UNHEX(HEX(AES_ENCRYPT(dname, '암호키'))), '암호키') from Dept; -- AES_DECRYPT, 복호화
...
-- null이 concat 무력화 -> concat_ws 로 null무시
...
select ifnull(captain, '공석'), if(captain is null, '공석', captain),
       ...
       NullIf(captain, 150) -- ifnull <-> nullif (true then make null)
  from Dept;
...
select insert('12345', 3, 2, '/');  -- spliced 유사?
...
select now(), sysdate(), curdate(), curtime();  -- now = sysdate
...
select DATE_FORMAT(DATE_ADD(NOW(), INTERVAL -1 DAY), '%Y-%m-%d %H:%i:%s'); -- i : minute
