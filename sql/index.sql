...
-- ex1) Emp 테이블에 mobile 이라는 컬럼 추가 varchar(11), 데이터 생성 010-0000-0000 
-- ex2) mobile 뒷자리 4개를 검색할 수 있는 functional index 생성하시오.

alter table Emp add column phoneNumber varchar(11) not null default '';
-- update Emp set phoneNumber = CASE WHEN id < 10 THEN concat('010-0000-000', id);
-- select phoneNumber from Emp lpad('0', id);
select id, concat('0101234', lpad(id, 4, '0')) from Emp order by id;
update Emp set phoneNumber = concat('0101234', lpad(id, 4, '0')) where id > 0;
explain select * from Emp where substring(phoneNumber, 8, 4) = '0252';

alter table Emp add index (
    (subdtring(phoneNumber, 8, 4))
);


-- fulltext index
-- 터미널 sql로 fulltext optimize 하는법 
-- 1. set global innodb_ft_aux_table = 'testdb/Notice'; -> optimize 대상 선택
-- 2. SET GLOBAL innodb_optimize_fulltext_only = ON;
-- 3. OPTIMIZE TABLE testdb.Notice;
-- 4. SET GLOBAL innodb_optimize_fulltext_only = OFF;
-- 기본 off 상테. only -> 나머지 index는 optimizing 안함. -> 때문에 한번 최적화 후 다시 끄는게 좋음 
...
CREATE TABLE Notice(
  ...
  -- optimizer는 주로 fulltext index, partition 등의 정리에 사용
  -- text type은 partition과 연관있음
  -- 왜?  text는 크기가 너무 커서(6535) 이 별도의 공간 할당 (파티션이 된다는  느낌)
);

ALTER TABLE Notice ADD CONSTRAINT fk_Notice_writer_Emp FOREIGN KEY (writer) REFERENCES Emp(id) ON DELETE SET NULL;
...
-- empty 상태의 경우, 에러발생 -> 터미널에서 위 코드로 실행시, empty 여부 확인가능
show variables like '%optimize%'; 

-- p.127 ex
select * from Notice where match(title, contents) against('문신*, 무신*' in boolean mode);
select * from Notice where match(title, contents) against('-후기*, 문신*, 무신*' in boolean mode);
select * from Notice where match(title, contents) against('+중기*, 문신*, 무신*' in boolean mode);
...
select * from intormation_schema.partitions;
...
insert into EmpTest(id, ename, dept, auth, salary, phoneNumber)
